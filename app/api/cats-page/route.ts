...
export async function POST() {
  try {
    const payload = await getPayload({ config })
    const current = await (payload as any).findGlobal({ slug: 'catsPage', depth: 1 })
    ...
    const updatedServices = (current?.services || []).map((s: any) => {
      if (s.id === 'sanroquero') {
        return {
          ...
          ctaHref: 'https://wa.me/573123114435?text=Hola%20SANROQUE%2C%20me%20gustar%C3%ADa%20agendar%20el%20servicio%20SanRoquero%20para%20mi%20michi%20en%20la%20nueva%20sede%20Calle%20118',
        }
      }
      if (s.id === 'rockstar') {
        return {
          ...
          ctaHref: 'https://wa.me/573123114435?text=Hola%20SANROQUE%2C%20me%20gustar%C3%ADa%20agendar%20el%20servicio%20Rockstar%20para%20mi%20michi%20en%20la%20nueva%20sede%20Calle%20118',
        }
      }
      return {
        ...s,
    ...
    return Response.json({ success: false, error: error?.message || 'Error updating catsPage' }, { status: 500 })
  }
}
